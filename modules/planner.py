...
### *별표 강조* 규칙 (억지 금지)
- 강조는 **숫자·금액·기한·필수/금지**만. 예: `*810억*`, `*D-7*`, `*필수*`
- **단어 중간 쪼개기 절대 금지** (예: 중소*기업*, 기*술에*, 미*신청 시* ← 전부 금지)
- 조사·어미만 강조 금지 (에/을/를/시 등)
- 애매하면 *별표 생략* — 흰색 본문이 억지 주황보다 낫다

### 타입 A (핵심 강조형) - 팩트를 세게 때릴 때
- content_variant: "A"
- explanations: 2~3개, 각 <=25자
- explanations 중 **숫자·기한이 있을 때만** *별표* (없으면 생략)
  예: "*코스피 5,500* 목표", "마감 *D-7* 고정" — `*기업*` 같은 일반명사 강조 금지
...
- detailed_lines: **문자열 배열 3~4개**, 각 항목 <=30자
  ...
  - detailed_lines: 숫자·기한 있을 때만 *별표* (없으면 전부 흰색)
- explanations: [] (비움)
